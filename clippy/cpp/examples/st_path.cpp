...
int main(int argc, char** argv) {
  ...
  clippy::clippy clip("st_path", "S-T path");
  ...
  if (active_edge_count > 0 && active_edge_count <= max_output_edge_count) {
    clip.to_return(output_edges_string);
  } else if (active_edge_count < 1) {
    clip.to_return("No path found"); 
  } else if (active_edge_count > max_output_edge_count) {
    clip.to_return(std::string("Warning: output is too large, edge count: ") + 
      std::to_string(active_edge_count));   
  } else {
    clip.to_return("Undefined");  
  }
