...
bool bfs_level_synchronous_single_source(VertexList& vertices, 
  ...
  std::cerr << source << " " << max_level_vertex << " " << max_level << " " 
    << visited_count << std::endl;    
  //std::cout << "BFS not visited count: " << not_visited_count << std::endl;
...
template <typename Vertex, typename Edge, typename VertexList, 
  typename EdgeList, typename EdgeStateList, typename Boolean,
  typename VertexBooleanList, typename BFSLevelList, 
  typename BFSPredecessorList> 
bool st_shortest_path(VertexList& vertices, 
  Vertex vertex_count, EdgeList& edges, Vertex source_vertex, Vertex target_vertex, 
  size_t max_depth, 
  BFSLevelList& bfs_level, BFSPredecessorList& bfs_predecessor, 
  EdgeStateList& edge_state_list, 
  size_t state_index, 
  VertexBooleanList& vertex_active, size_t& max_depth_searched) {

  //std::cout << "BFS source: " << source_vertex << std::endl;
  ...
  //Vertex source = 0;
  ///visited[static_cast<size_t>(source)] = 1;  
  bfs_level[static_cast<size_t>(source_vertex)][state_index] = 0;
  size_t current_level = bfs_level[static_cast<size_t>(source_vertex)][state_index];

  bool finished = false;
  bool target_vertex_found = false; // TODO: change it to found source/sink/target 
  ...
  do {
    ...
    for (Vertex v = 0; v < vertex_count; v++) {
      ...
      if ///(visited[static_cast<size_t>(v)] == 1 && 
        ...
        for (Edge e = vertices[v]; e < vertices[v + 1]; e++) {
          ...
          if (bfs_level[static_cast<size_t>(v_nbr)][state_index] == 
            ...
            bfs_level[static_cast<size_t>(v_nbr)][state_index] = 
              current_level + 1;
            bfs_predecessor[static_cast<size_t>(v_nbr)] = v; // predecessor
      ...
      if (target_vertex_found) {
        max_depth_searched = current_level + 1;  
        finished = true; 
        break;
      } 
    ...
    if (current_level == max_depth) {
      ...
      current_level++;
      max_depth_searched = current_level;
    }
  ...
  std::cerr << source_vertex << " " << max_level_vertex << " " << max_level << " " 
    << visited_count << std::endl;    
  //std::cout << "BFS not visited count: " << not_visited_count << std::endl;
...
void eliminate_non_path_vertices_undirected(Vertex vertex_count, 
    ...
    std::cerr << "Max out degree (log2): " << max_vertex_log_degree 
      << std::endl; 
    ...
    do {
      ...
      std::cerr << "Iteration: " << iteration_count 
        << ", deleted vertex count: " << deleted_vertex_count << std::endl;
      iteration_count++; 
...
int main(int argc, char** argv) {
  ...
  clippy::clippy clip("st_shortest_path", "S-T shortest path");

  clip.add_required<clippy::arraystring>("filenames",
    "Input and output filenames");
  clip.returns<clippy::arraystring>("Array of strings"); 
  
  clip.add_required<clippy::arraystring>("stvertices", "Source and target vertices");

  //clip.add_required<clippy::arraystring>("filtervertexlabels", "Filter - vertex labels");
  clip.add_optional<clippy::arraystring>("filtervertexlabels", "Filter - vertex labels", std::vector<std::string>(1, "unspecified"));

  clip.add_optional<clippy::string>("searchdepth", "Search depth", "100");  

  clip.returns<clippy::string>("Message");

  if (clip.parse(argc, argv)) { return 0; } 

  auto filenames = clip.get<clippy::arraystring>("filenames");
  //clip.to_return(filenames);
  
  auto st_vertices_string = clip.get<clippy::arraystring>("stvertices");

  auto filter_veretx_labels_string = clip.get<clippy::arraystring>("filtervertexlabels");

  auto search_depth_string = clip.get<clippy::string>("searchdepth"); 

  //std::cerr << "Program log" << std::endl;  

  //clip.to_return(std::string("S-T path at depth ") + search_depth_string);
  //clip.to_return(filenames[0] + " " + st_vertices_string[0] + " " + 
  //  filter_veretx_labels_string[0] + " " + search_depth_string); 
  //clip.to_return(st_vertices_string); 
  ...
  // bfs
  size_t bfs_max_depth = std::numeric_limits<uint64_t>::max(); //std::numeric_limits<size_t>::max();  
  ...
  edge_input_filename = filenames[0];
  vertex_data_input_filename = filenames[1];
 
  u_vertices.push_back(std::stoull(st_vertices_string[0]));
  v_vertices.push_back(std::stoull(st_vertices_string[1]));
  
  for (auto i : filter_veretx_labels_string) {
    if (i != "unspecified") { 
      vertex_data_filter.insert(std::stoull(i)); 
    }
  }

  bfs_max_depth = std::stoull(search_depth_string);  

  end_time = std::chrono::steady_clock::now();
  elapsed_time = getElapsedTimeSecond(start_time, end_time);
  std::cerr << "Time: " << elapsed_time << " seconds." << std::endl;
  ...
  //  host_thread_count = omp_get_num_threads();
  //}
  std::cerr << "Number of available threads on " << hostname << ": " 
  ...
  if (thread_count > 0) {
    ...
    std::cerr << "Number of threads to be used: " << host_thread_count << std::endl;
    } 
  }
  ...
  typedef std::vector<Edge> VertexList; // CSR indices
  ...
  std::cerr << "Building in-memory CSR graph (undirected) ..." << std::endl;
  ...
  std::cerr << "Reading edge list file ..." << std::endl;
  edge_count = read_edge_list_file<Vertex, Edge, EdgeListTuple>
    (edge_input_filename, edge_list, max_vertex, skip_lines_count);
  std::cerr << "Size of edge list: " << edge_list.size() << std::endl;
  std::cerr << "Max vertex: " << max_vertex << std::endl;

  // sort edges by source
  std::cerr << "Sorting edges by source vertex ..." << std::endl;
  std::stable_sort(edge_list.begin(), edge_list.end(),
    [](const std::tuple<Vertex, Vertex>& a,
       const std::tuple<Vertex, Vertex>& b) -> bool {
         return std::get<0>(a) < std::get<0>(b);
  ...
  // generate vetex list
  //std::cout << "Generating vertex list ..." << std::endl;
  std::cerr << "Creating CSR row pointers ..." << std::endl;
  vertex_count = generate_vertex_list(edge_list, vertices, vertex_degree, max_vertex);
  std::cerr << "Size of vertex list: " << vertices.size() << std::endl;
  std::cerr << "Size of vertex degree list: " << vertex_degree.size() << std::endl;

  // sort targets in increasing order
  std::cerr << "Sorting neighbors ..." << std::endl;
  {
  #pragma omp parallel for
  for (size_t v = 0; v < vertices.size() - 1; v++) {
    size_t start = vertices[v];
  ...
  generate_edge_list(edge_list, edges);
  std::cerr << "Size of edge list: " << edges.size() << std::endl;
  ...
  std::cerr << "CSR Graph generation completed." << std::endl;
  std::cerr << "Number of vertices: " << vertex_count << std::endl;
  std::cerr << "Number of edges: " << edge_count << std::endl;  
  std::cerr << "Max vertex: " << max_vertex << std::endl;
  ...
  std::cerr << "Reading vertex data file ..." << std::endl;
  ...
  std::cerr << "Size of vertex data list: " << vertex_data.size() << std::endl;
  ...
  ////std::cerr << "Size of edge state list: " << edge_state_list.size() << ", " << 
  ...
  double global_elapsed_time = getElapsedTimeSecond(global_start_time, 
    global_end_time);
  std::cerr << "Time: " << global_elapsed_time << " seconds." 
    << std::endl;
  ...
  std::cerr << "Selected source and target: " << std::endl;
  ...
  std::copy(v_vertices.begin(), v_vertices.end(), std::inserter(uv_vertices, 
    uv_vertices.end()));
  //std::cerr << uv_vertices.size() << std::endl;
  ...
  std::cerr << "Selected veretx data: " << std::endl;
  ...
  vertex_active.resize(vertex_count, static_cast<Boolean>(1)); // TODO: improve 
  ...
  for (Vertex v = 0; v < vertex_count; v++) {
    ...
    if (find_uv_vertex != uv_vertices.end()) {          
      ...
      if (find_veretx_data != vertex_data_filter.end()) {
  ...
  std::cerr << "Eliminating non-path vertices ... " << std::endl;
  ...
  //vertex_active.resize(vertex_count, static_cast<Boolean>(1)); // Test 
  ...
  // Test
  {  
  std::cerr << "Size of vertex active list: " << vertex_active.size() 
    << std::endl;
  ...
  std::cerr << "Active vertex count: " << tmp_active_vertex_count << std::endl;
  std::cerr << "Active edge count: " << tmp_active_edge_count << std::endl;
  std::cerr << std::endl; 
  }
  // Test    
  ...
  std::cerr << "Searching s-t shortest path ... " << std::endl;

  std::cerr << "Source vertices: ";
  for (auto& v: u_vertices) {
    std::cerr << v << " " << static_cast<size_t>(vertex_active[v]) << ", ";
  }
  std::cerr << std::endl;

  std::cerr << "Target vertices: ";
  ...
  typedef EdgeList BFSPredecessorList; // EdgeList - std::vector<uint64_t> 
  BFSPredecessorList bfs_predecessor(vertex_count, 
    std::numeric_limits<uint64_t>::max());

  bfs_max_depth = bfs_max_depth - 1; // the code computes current_level + 1 neighbors

  size_t bfs_max_depth_searched = std::numeric_limits<uint64_t>::max();

  Boolean target_vertex_found = false;
 
  global_start_time = std::chrono::steady_clock::now();

  target_vertex_found = st_shortest_path<Vertex, Edge, VertexList,
    EdgeList, EdgeStateList, Boolean, VertexBooleanList, BFSLevelList, 
    BFSPredecessorList>
    (vertices, vertex_count, edges, 
    static_cast<Vertex>(u_vertices[0]), static_cast<Vertex>(v_vertices[0]),
    bfs_max_depth, bfs_level, bfs_predecessor, edge_state_list, 0,
    vertex_active, bfs_max_depth_searched);
  ...
  // Test
  size_t tmp_edge_state_size = 1;
  std::cerr << std::endl; 
  for (size_t i = 0; i < tmp_edge_state_size; i++) {
    std::cerr << u_vertices[0] << " " << 
  ...
  if (target_vertex_found) {
    ...
    std::vector<std::string> output_edges_string;

    Vertex source_vertex = static_cast<Vertex>(u_vertices[0]);
    Vertex target_vertex = static_cast<Vertex>(v_vertices[0]);
    Vertex current_path_vertex = target_vertex; 
    size_t current_level = bfs_level[static_cast<size_t>(target_vertex)][0];    

    std::cerr << target_vertex << " found at depth " << 
      bfs_level[static_cast<size_t>(target_vertex)][0] << std::endl;

    do {
      Vertex predecessor_vertex = bfs_predecessor[static_cast<size_t>(current_path_vertex)];

      if (static_cast<uint64_t>(predecessor_vertex) == std::numeric_limits<uint64_t>::max()) {
        std::cerr << "Error: Invalid predecessor ID" << std::endl;  
        break;
      } 
 
      //std::cerr << current_path_vertex << " " << predecessor_vertex 
      //  << std::endl;
      output_edges_string.push_back(std::to_string(current_path_vertex) + " " +
        std::to_string(predecessor_vertex));

      current_level = bfs_level[static_cast<size_t>(predecessor_vertex)][0];
    ...
    clip.to_return(output_edges_string); // TODO: max_output_edge_count

  } else {
    std::cerr << static_cast<Vertex>(v_vertices[0]) << 
      " was not found at maximum depth " << 
      bfs_max_depth_searched << std::endl; // TODO: current_level - 1
    clip.to_return("No path found");	
  }
  ...
  std::cerr << "Searching paths ... " << std::endl;
  ...
  std::cerr << "Reset states ... " << std::endl;
  ...
  std::cerr << "BFS tree intersection ... " << std::endl;
  ...
  // Test
  std::cerr << "Edges processed: " << edges_processed 
  ...
  std::cerr << "Eliminate invalid edges from the BFS intersection tree ... " 
    << std::endl;
  ...
  std::vector<std::string> output_edges_string;
  size_t max_output_edge_count = 300;
  ...
  for (Vertex v = 0; v < vertex_count; v++) {
    ...
    for (Edge e = vertices[v]; e < vertices[v + 1]; e++) {
      ...
      if (vertex_active[v_nbr] == static_cast<Boolean>(0)) {
        ...
        //if (v < v_nbr) {                    
          if (active_edge_count <= max_output_edge_count) {  
            //std::cerr << v << " " << v_nbr << std::endl;       
            output_edges_string.push_back(std::to_string(v) + " " + 
              std::to_string(v_nbr));   
          }
          active_edge_count++; 
  ...
  std::cerr << "Active vertex count: " << active_vertex_count << std::endl;
  std::cerr << "Active edge count: " << active_edge_count << std::endl;
  std::cerr << "Active edge count (output): " << output_edges_string.size() <<std::endl;
  //std::cerr << std::endl;   


  if (active_edge_count <= max_output_edge_count) {
    clip.to_return(output_edges_string);
  } else {
    clip.to_return(std::string("Warning: output is too large, edge count: ") + 
      std::to_string(active_edge_count));   
  } 
