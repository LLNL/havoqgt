...
class mailbox_p2p_nlroute {
 ...
 public:
  mailbox_p2p_nlroute(RecvHandlerFunc recv_func, const size_t batch_size,
		      const int tag = 1)
 ...
 private:
  ...
  void do_send_bcast(const Data& data) {
    for (uint32_t j = 0; j < comm_nl().size(); j++) {
      ...
      m_send_count += m_local_exchanger.queue_bytes(j, message{1, 0, 0, 0, data});
      //m_local_exchanger.queue(j, message{1, 0, 0, 0, data});
      //++m_send_count;
    }
    for (uint32_t i = 0; i < comm_nr().size(); i++) {
      if (i == comm_nr().rank()) continue;
      m_send_count += m_remote_exchanger.queue_bytes(i, message{1, 0, 0, 0, data});
      //m_remote_exchanger.queue(i, message{1, 0, 0, 0, data});
      //++m_send_count;
    }
  }
  ...
  uint64_t do_exchange() {
    ...
    total += m_local_exchanger.exchange(
        [&](const message& msg) {
          ...
          } else if (msg.local == comm_nl().rank() && msg.node == comm_nr().rank()) {
    ...
    total += m_remote_exchanger.exchange(
        [&](const message &msg) { m_recv_func(this, msg.bcast, msg.data); }, total);
    m_send_count = 0;
