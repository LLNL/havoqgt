...
public:
  ...
  std::pair<bool, interval> get_random_weighted_edge(operation& op, interval cur_time, vertex_locator cur_vertex) {
    ...
    auto it_op=[&list, cur_time, this]( typename std::vector<eitr_type>::iterator begin, typename std::vector<eitr_type>::iterator end ) -> bool {
      auto *edge_data = this->edge_metadata;
      std::transform( begin, end, std::back_inserter(list), [&cur_time, edge_data]( eitr_type idx ){
	  return (double)1.0 / ( (double)( cur_time.first - ((*edge_data)[idx]).start_time()) );
	});
      return true;
    };
    tree.query(1, cur_time.first, cur_time.second, it_op);
    if(list.size() == 0) return std::make_pair(false, cur_time);
    std::discrete_distribution<uint32_t> discrete_dist( list.begin(), list.end() );
    uint32_t index;
    {
      index = (rng::get_rng())(discrete_dist);
    }
    eitr_type itr = g->edges_begin(cur_vertex);
    tree.query_ith(cur_time.first, cur_time.second, static_cast<std::size_t>(index), itr);
