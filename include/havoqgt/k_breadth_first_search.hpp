...
#pragma GCC diagnostic pop
  bfs_visitor(vertex_locator _vertex, uint64_t _level, vertex_locator _parent, vertex_locator _root)
    : vertex(_vertex)
    , m_parent(_parent)
    , m_root(_root)
    , m_level(_level) { }
  ...
  bool pre_visit(AlgData& alg_data) const {
    bool do_visit = std::get<0>(alg_data)[vertex][root()] > level();
    if(do_visit) {
      std::get<0>(alg_data)[vertex][root()] = level();
    }
    return do_visit;
  }
  ...
  bool visit(Graph& g, VisitorQueueHandle vis_queue, AlgData& alg_data) const {
    if(level() <= std::get<0>(alg_data)[vertex][root()]) {
      std::get<0>(alg_data)[vertex][root()] = level();
      std::get<1>(alg_data)[vertex][root()] = parent();
  ...
  vertex_locator root() const  { return m_root; }
...
void breadth_first_search(TGraph* g,
  ...
  vq.init_visitor_traversal_multiple_source(source_list);
}
