...
public:
  ...
  bool restart_walker(bool has_neighbor) const {
    if( has_neighbor ) {
      auto random_percentage = random_number_generator::get_rng().uniform_int( (uint32_t)100 );
      return random_percentage < prob_restart;
    }
    return true;
  }
  ...
  std::tuple<bool, vertex_locator, simple_random_walker> next(vertex_locator cur_vertex ) const{
    ...
    bool restart_rw = restart_walker( next.first );
    if( restart_rw ) {
      if( attempts >= max_attempts ) { }//return std::make_tuple( false, vertex_locator(),  simple_random_walker() );
      else {
	simple_random_walker restart_rw( id, start_from, started_at, cur_time, 0, 0, attempts + 1 , total_steps + 1);
    ...
    simple_random_walker next_rw(id, start_from, started_at, next.second, cost + edge_cost, steps + 1, attempts, total_steps + 1);
    return std::make_tuple( true , edge_target, next_rw );
  }
...
template<typename MetaDataType, typename VertexType, typename RandomEdgeContainer>
uint32_t simple_random_walker<MetaDataType, VertexType, RandomEdgeContainer>::prob_restart = 2;
