...
delegate_partitioned_graph<SegmentManager>::delegate_partitioned_graph(
    const SegmentAllocator<void>& seg_allocator, MPI_Comm mpi_comm,
  ...
  delegate_partitioned_graph<SegmentManager>::edge_data<
      uint8_t, SegmentAllocator<uint8_t>>
      _edge_data = delegate_partitioned_graph<SegmentManager>::edge_data<
          uint8_t, SegmentAllocator<uint8_t>>(seg_allocator);
...
delegate_partitioned_graph<SegmentManager>::delegate_partitioned_graph(
  ...
  static_assert(
      std::is_same<edge_type,
                   std::tuple<uint64_t, uint64_t, edge_data_value_type>>::value,
...
template <typename SegmentManager>
template <typename Container, typename edge_data_type>
void delegate_partitioned_graph<SegmentManager>::complete_construction(
    const SegmentAllocator<void>& seg_allocator, MPI_Comm mpi_comm,
  ...
  switch (m_graph_state) {
    ...
    case HighEdgesPartitioned: {
      ...
      if (m_delegate_degree.size() > 0) {
        mpi_all_reduce_inplace(m_delegate_degree, std::plus<uint64_t>(),
                               m_mpi_comm);
      }
      MPI_Barrier(m_mpi_comm);
    }
      ...
      // Build controller lists
      {
        ...
        for (size_t i = 0; i < m_delegate_degree.size(); ++i) {
          if (int(i % m_mpi_size) == m_mpi_rank) {
            m_controller_locators.push_back(
                vertex_locator(true, i, m_mpi_rank));
          }
        }
        MPI_Barrier(m_mpi_comm);
      }
...
void delegate_partitioned_graph<SegmentManager>::count_edge_degrees(
    InputIterator unsorted_itr, InputIterator unsorted_itr_end,
    boost::unordered_set<uint64_t>& global_hubs,
  ...
  while (!detail::global_iterator_range_empty(unsorted_itr, unsorted_itr_end,
    ...
    if (m_mpi_rank == 0 && (loop_counter % 1000) == 0) {
      ...
      std::cout << "\t["
                ...
                << "Total Edges: " << edge_counter << "] Time "
                << (curr_time - last_loop_time) << " second, "
    ...
    for (size_t i = 0; i < edge_chunk_size && unsorted_itr != unsorted_itr_end;
         i++) {
      edge_counter++;
      auto edge = std::make_pair(std::get<0>(*unsorted_itr),
    ...
    send_vertex_info(high_vertex_count, delegate_degree_threshold, maps_to_send,
                     maps_to_send_element_count);
 ...
 */
template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::send_vertex_info(
    uint64_t& high_vertex_count, uint64_t delegate_degree_threshold,
    std::vector<boost::container::map<int, std::pair<uint64_t, uint64_t>>>&
        maps_to_send,
  ...
  for (size_t i = 0; i < maps_to_send.size(); i++) {
    for (auto itr = maps_to_send[i].begin(); itr != maps_to_send[i].end();
         itr++) {
  ...
  for (size_t k = 0; k < to_recv.size();) {
    ...
    if (m_local_outgoing_count[local_id] < delegate_degree_threshold &&
        m_local_outgoing_count[local_id] +
                source_count >=  // changed from dest_count to source_count
            delegate_degree_threshold) {
      high_vertex_count++;
    }
 ...
 */
template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::initialize_low_meta_data(
    boost::unordered_set<uint64_t>& global_hubs) {
 ...
 */
template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::initialize_high_meta_data(
    boost::unordered_set<uint64_t>& global_hubs) {
  //
...
template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::initialize_edge_storage(
  ...
  for (int i = 0; i < processes_per_node; i++) {
    ...
    if (i == m_mpi_rank % processes_per_node) {
...

      {
        ...
        m_delegate_targets = (vertex_locator*)segment_manager->allocate(
            m_delegate_targets_size * sizeof(vertex_locator));
      }

      {
        ...
        m_owned_targets = (vertex_locator*)segment_manager->allocate(
            m_owned_targets_size * sizeof(vertex_locator));
      }
...
constexpr std::size_t get_edge_data_tuple_index(
  ...
  return ((is_edge_pair_type == true) || (is_edge_two_tuple_type == true)) ? 0
                                                                           : 2;
...
template <typename SegmentManager>
template <typename Container, typename edge_data_type>
void delegate_partitioned_graph<SegmentManager>::partition_low_degree(
    Container& unsorted_edges, edge_data_type& _edge_data) {
  typedef typename Container::value_type edge_type;
  typedef std::pair<uint64_t, uint64_t> edge_pair_type;
  constexpr bool is_edge_pair_type =
      std::is_same<edge_type, edge_pair_type>::value;
  typedef std::tuple<uint64_t, uint64_t> edge_two_tuple_type;
  constexpr bool is_edge_two_tuple_type =
      std::is_same<edge_type, edge_two_tuple_type>::value;
  ...
  for (size_t node_turn = 0; node_turn < node_partitions; node_turn++) {
    ...
    if (m_mpi_rank == 0) {
      ...
      std::cout << "\t***[LP]["
                ...
                << "Total Loops: " << loop_counter << ", "
                << "Total Edges: " << edge_counter << "] Time "
                << (curr_time - last_part_time) << " second, "
                << " Total Time " << (curr_time - start_time) << " second"
                << std::endl
                << std::flush;
    ...
    while (!detail::global_iterator_range_empty(unsorted_itr, unsorted_itr_end,
...

      {
        ...
        for (size_t i = 0;
             ...
             ++unsorted_itr, ++i) {
          // Get next edge
          const auto edge = std::make_pair(std::get<0>(*unsorted_itr),
                                           std::get<1>(*unsorted_itr));
      ...
      for (size_t i = 0; i < to_recv_edges_low.size(); ++i) {
        auto edge = std::make_pair(std::get<0>(to_recv_edges_low[i]),
                                   std::get<1>(to_recv_edges_low[i]));
      ...
      for (auto itr = to_recv_edges_low.begin(); itr != itr_end; itr++) {
        auto edge = std::make_pair(std::get<0>(*itr),
                                   std::get<1>(*itr));  // std::get<0>(*itr);
        ...
        if (!(loc < m_owned_info[new_vertex_id + 1].low_csr_idx)) {
          std::cout << "Error rank: " << m_mpi_rank << " -- " << loc << " < "
                    << m_owned_info[new_vertex_id + 1].low_csr_idx
                    << ", new_vertex_id = " << new_vertex_id
                    << ", temp_offset = " << temp_offset << ", edge = ("
                    << edge.first << "," << edge.second << ")" << std::endl
                    << std::flush;
          assert(false);
          exit(-1);
        }
        /// @todo this was tripping, is this old?   Could be left over from
        /// before when targets was vector based.
        // assert(!m_owned_targets[loc].is_valid());

        m_owned_targets[loc] = label_to_locator(edge.second);
        if (m_has_edge_data) {
          _edge_data.m_owned_edge_data[loc] =
              std::get<get_edge_data_tuple_index(is_edge_pair_type,
                                                 is_edge_two_tuple_type)>(*itr);
        }
...
template <typename SegmentManager>
template <typename InputIterator>
void delegate_partitioned_graph<SegmentManager>::count_high_degree_edges(
    InputIterator unsorted_itr, InputIterator unsorted_itr_end,
...
// Used to store high_edge count
  ...
  while (!detail::global_iterator_range_empty(unsorted_itr, unsorted_itr_end,
    ...
    std::vector<boost::container::map<uint64_t, uint64_t>> maps_to_send(
        m_mpi_size);
    int maps_to_send_element_count = 0;
    {
      for (size_t i = 0;
        ...
        if (global_hub_set.count(edge.first) == 0) {
          ...
          if (owner == m_mpi_rank) {
            ...
            const uint64_t new_source_id =
                m_map_delegate_locator[ver_id].local_id();
 ...
 */
template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::send_high_info(
    std::vector<boost::container::map<uint64_t, uint64_t>>& maps_to_send,
    int maps_to_send_element_count) {
  int                   to_send_pos = 0;
 ...
 */
template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::calculate_overflow(
    std::map<uint64_t, std::deque<OverflowSendInfo>>& transfer_info) {
  //
  ...
  uint64_t       global_edge_count =
      mpi_all_reduce(owned_total_edges, std::plus<uint64_t>(), m_mpi_comm);
  ...
  for (; heavy_idx < m_mpi_size && light_idx < m_mpi_size; ++heavy_idx) {
    while (low_count_per_rank[heavy_idx] + high_count_per_rank[heavy_idx] >
      ...
      const int64_t total_edges_low_idx =
          low_count_per_rank[light_idx] + high_count_per_rank[light_idx];
      if (total_edges_low_idx < target_edges_per_rank) {
        // if the low_idx needs edges

        if (high_count_per_rank[heavy_idx] == 0) {
          // If the heavy_idx has no more edges to give then break the while
          // loop
        ...
        if (heavy_idx == m_mpi_rank) {  // This node is sending
          ...
          CHK_MPI(MPI_Send(&send_len, 1, mpi_typeof(send_len), light_idx, 0,
                           m_mpi_comm));
  ...
  uint64_t       sanity_global_edge_count =
      mpi_all_reduce(owned_total_edges2, std::plus<uint64_t>(), m_mpi_comm);
 ...
 */
template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::generate_send_list(
    std::vector<uint64_t>& send_list, uint64_t num_send, int send_id,
    std::map<uint64_t, std::deque<OverflowSendInfo>>& transfer_info) {
  // Tracking variables used to determine how much space to allocate.
  uint64_t send_count = 0;
  uint64_t ver_count  = 0;
  for (uint64_t i = 0;
       i < m_delegate_info.size() - 1 && send_count < num_send;) {
 ...
 * destination. If this node is giving edges to another node, then when this
 * node has
...
template <typename SegmentManager>
template <typename Container, typename edge_data_type>
void delegate_partitioned_graph<SegmentManager>::partition_high_degree(
    Container& unsorted_edges,
    std::map<uint64_t, std::deque<OverflowSendInfo>>& transfer_info,
  ...
  typedef typename edge_data_type::value_type edge_data_value_type;
  typedef std::tuple<uint64_t, uint64_t, edge_data_value_type>
      edge_type_comm;  // used by the communication routines

  // Initates the paritioner, which determines where overflowed edges go
  high_edge_partitioner<edge_type_comm> paritioner(m_mpi_size, m_mpi_rank,
                                                   &transfer_info);
  ...
  for (size_t node_turn = 0; node_turn < node_partitions; node_turn++) {
    ...
    while (!detail::global_iterator_range_empty(unsorted_itr, unsorted_itr_end,
      ...
      while (unsorted_itr != unsorted_itr_end &&
        ...
        const auto edge = std::make_pair(std::get<0>(*unsorted_itr),
                                         std::get<1>(*unsorted_itr));
        const auto edge_data = std::get<get_edge_data_tuple_index(
            is_edge_pair_type, is_edge_two_tuple_type)>(*unsorted_itr);
        ++unsorted_itr;
        ...
        if (m_map_delegate_locator.count(edge.first) == 1) {
          ...
          assert(new_source_id >= 0 &&
                 new_source_id < m_delegate_info.size() - 1);

          // Send the edge if we don't own it or if we own it but have no room.

          if (!m_has_edge_data) {
            to_send_edges_high.push_back(std::forward_as_tuple(
                new_source_id, edge.second, (uint64_t)(0)));
          } else {
            to_send_edges_high.push_back(
                std::forward_as_tuple(new_source_id, edge.second, edge_data));
          }
        }  // end if is a hub
        else {
          // assert(global_hub_set.count(edge.first) == 0);
      ...
      for (size_t i = 0; i < to_recv_edges_high.size(); ++i) {
        ...
        // above
        const auto edge = std::make_pair(std::get<0>(to_recv_edges_high[i]),
                                         std::get<1>(to_recv_edges_high[i]));
        ...
        if (place_pos == m_delegate_info[new_source_id + 1]) {
          ...
          if (m_has_edge_data) {
            _edge_data.m_delegate_edge_data[place_pos] =
                std::get<get_edge_data_tuple_index(is_edge_pair_type,
                                                   is_edge_two_tuple_type)>(
  ...
  {  //
     // Exchange edges we generated  with the other nodes and recieve edges we
     // may need
    // // Scratch vector use for storing recieved edges.
    std::vector<edge_type_comm> to_recv_edges_high;
    mpi_yield_barrier(m_mpi_comm);
    ...
    for (size_t i = 0; i < to_recv_edges_high.size(); ++i) {
      ...
      const auto edge = std::make_pair(
          std::get<0>(to_recv_edges_high[i]),
          std::get<1>(
              to_recv_edges_high[i]));  // std::get<0>(to_recv_edges_high[i]);
      const uint64_t new_source_id = edge.first;
 ...
 */
template <typename SegmentManager>
inline uint64_t delegate_partitioned_graph<SegmentManager>::locator_to_label(
    delegate_partitioned_graph<SegmentManager>::vertex_locator locator) const {
  uint64_t res;
 ...
 */
template <typename SegmentManager>
inline typename delegate_partitioned_graph<SegmentManager>::vertex_locator
delegate_partitioned_graph<SegmentManager>::label_to_locator(
 ...
 */
template <typename SegmentManager>
inline void delegate_partitioned_graph<SegmentManager>::sync_global_hub_set(
    const boost::unordered_set<uint64_t>& local_hubs,
    boost::unordered_set<uint64_t>& global_hubs, bool local_change) {
  uint32_t new_hubs =
      mpi_all_reduce(uint32_t(local_change), std::plus<uint32_t>(), m_mpi_comm);
 ...
 */
template <typename SegmentManager>
inline typename delegate_partitioned_graph<SegmentManager>::edge_iterator
delegate_partitioned_graph<SegmentManager>::edges_begin(
    delegate_partitioned_graph<SegmentManager>::vertex_locator locator) const {
  if (locator.is_delegate()) {
...
template <typename SegmentManager>
inline typename delegate_partitioned_graph<SegmentManager>::edge_iterator
delegate_partitioned_graph<SegmentManager>::edges_end(
    delegate_partitioned_graph<SegmentManager>::vertex_locator locator) const {
  if (locator.is_delegate()) {
    assert(locator.local_id() + 1 < m_delegate_info.size());
    return edge_iterator(locator, m_delegate_info[locator.local_id() + 1],
                         this);
  }
  assert(locator.owner() == m_mpi_rank);
  assert(locator.local_id() + 1 < m_owned_info.size());
  return edge_iterator(locator,
                       m_owned_info[locator.local_id() + 1].low_csr_idx, this);
}
 ...
 */
template <typename SegmentManager>
inline uint64_t delegate_partitioned_graph<SegmentManager>::degree(
    delegate_partitioned_graph<SegmentManager>::vertex_locator locator) const {
 ...
 */
template <typename SegmentManager>
inline uint64_t delegate_partitioned_graph<SegmentManager>::local_degree(
    delegate_partitioned_graph<SegmentManager>::vertex_locator locator) const {
  uint64_t local_id = locator.local_id();
  if (locator.is_delegate()) {
...
template <typename SegmentManager>
inline typename delegate_partitioned_graph<SegmentManager>::vertex_iterator
delegate_partitioned_graph<SegmentManager>::vertices_begin() const {
  return vertex_iterator(0, this);
}

template <typename SegmentManager>
inline typename delegate_partitioned_graph<SegmentManager>::vertex_iterator
delegate_partitioned_graph<SegmentManager>::vertices_end() const {
  ...
  // while(last_index > 1 && (m_owned_info[last_index-1].low_csr_idx ==
  // m_owned_info[last_index].low_csr_idx)) {
  //  --last_index;
  //}
...
template <typename SegmentManager>
inline typename delegate_partitioned_graph<SegmentManager>::vertex_iterator
delegate_partitioned_graph<SegmentManager>::delegate_vertices_begin() const {
  return vertex_iterator(0, this, true);
}

template <typename SegmentManager>
inline typename delegate_partitioned_graph<SegmentManager>::vertex_iterator
delegate_partitioned_graph<SegmentManager>::delegate_vertices_end() const {
  return vertex_iterator(m_delegate_info.size() - 1, this, true);
}

template <typename SegmentManager>
inline bool delegate_partitioned_graph<SegmentManager>::is_label_delegate(
    uint64_t label) const {
...
typename delegate_partitioned_graph<SegmentManager>::template edge_data<
    T, SegManagerOther>*
delegate_partitioned_graph<SegmentManager>::create_edge_data(
    SegManagerOther* segment_manager_o, const char* obj_name) const {
  typedef typename delegate_partitioned_graph<
      SegmentManager>::template edge_data<T, SegManagerOther>
      mytype;

  if (obj_name == nullptr) {
    return segment_manager_o->template construct<mytype>(
        bip::anonymous_instance)(m_owned_targets_size, m_delegate_targets_size,
                                 segment_manager_o);
  } else {
    return segment_manager_o->template construct<mytype>(obj_name)(
        m_owned_targets_size, m_delegate_targets_size, segment_manager_o);
  }
}
...
delegate_partitioned_graph<SegmentManager>::create_edge_data(
    const T& init, SegManagerOther* segment_manager_o,
    const char* obj_name) const {
  typedef delegate_partitioned_graph<SegmentManager>::edge_data<T,
                                                                SegManagerOther>
      mytype;
  ...
  if (obj_name == nullptr) {
    return segment_manager_o->template construct<mytype>(
        bip::anonymous_instance)(m_owned_targets_size, m_delegate_targets_size,
                                 init, segment_manager_o);
  } else {
    return segment_manager_o->template construct<mytype>(obj_name)(
        m_owned_targets_size, m_delegate_targets_size, init, segment_manager_o);
  }
}

template <typename SegmentManager>
void delegate_partitioned_graph<SegmentManager>::print_graph_statistics() {
  /*if(m_mpi_rank == 0) {
    for(size_t i=0; i<m_delegate_degree.size(); ++i) {
  ...
  uint64_t low_max_size =
      mpi_all_reduce(low_local_size, std::greater<uint64_t>(), MPI_COMM_WORLD);
  uint64_t high_max_size =
      mpi_all_reduce(high_local_size, std::greater<uint64_t>(), MPI_COMM_WORLD);
  uint64_t total_max_size = mpi_all_reduce(
      total_local_size, std::greater<uint64_t>(), MPI_COMM_WORLD);
  uint64_t low_sum_size =
      mpi_all_reduce(low_local_size, std::plus<uint64_t>(), MPI_COMM_WORLD);
  uint64_t high_sum_size =
      mpi_all_reduce(high_local_size, std::plus<uint64_t>(), MPI_COMM_WORLD);
  uint64_t total_sum_size =
      mpi_all_reduce(total_local_size, std::plus<uint64_t>(), MPI_COMM_WORLD);
  ...
  uint64_t total_count_del_target = mpi_all_reduce(
      local_count_del_target, std::plus<uint64_t>(), MPI_COMM_WORLD);
  ...
  if (m_mpi_rank == 0) {
    std::cout << "========================================================"
              ...
              << "\tHub vertices = " << m_map_delegate_locator.size()
              << std::endl
              << "\tTotal percentage good hub edges = "
              << double(high_sum_size) / double(total_sum_size) * 100.0
              << std::endl
              << "\tTotal count del target = " << total_count_del_target
              << std::endl
              << "\tTotal percentage of localized edges = "
              << double(high_sum_size + total_count_del_target) /
                     double(total_sum_size) * 100.0
              << std::endl
              ...
              << "\toned imbalance = "
              << double(low_max_size) / double(low_sum_size / m_mpi_size)
              << std::endl
              << "\thubs imbalance = "
              << double(high_max_size) / double(high_sum_size / m_mpi_size)
              << std::endl
              << "\tTOTAL imbalance = "
              << double(total_max_size) / double(total_sum_size / m_mpi_size)
              << std::endl
