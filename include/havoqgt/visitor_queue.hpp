...
class visitor_queue {
  ...
  class mailbox_recv {
    ...
    void operator()(bool bcast, const visitor_type& data) {
  ...
  typedef mailbox_p2p_nrroute<visitor_type,mailbox_recv> mailbox_type;
...
public:
  ...
  //     assert(m_vq->m_ptr_graph->master(__value.m_visitor.vertex) != uint32_t(m_vq->m_world_rank));
    ...
    do {
      ...
      while(!empty()) {
        ...
        do_visit(this_visitor);
              ...
              || !empty() || !m_p_mailbox->global_empty());
  }
...
private:
  ...
  /*oid delegate_parent(const visitor_type& v) {
    uint64_t parent = offset_tree_parent(m_world_size, 2, m_ptr_graph->master(v.vertex),
                                m_world_rank);
    visitor_wrapper vw;
    vw.m_visitor = v;
