...
class gc_dynamic {
 public:
  ...
  bool visit(Graph& graph, VisitorQueueHandle vis_queue, AlgData&) const {
    switch (vis_type) {
      case ADD: {
...
void graph_colour_dynamic(TGraph* graph, edgelist_t* edgelist, bool test_result) {
...

  {
    ...
    char dummy_algo_data;
    auto colourer = havoqgt::create_visitor_queue<colourer_t, fifo_queue>(graph, dummy_algo_data);
    colourer.init_dynamic_traversal(edgelist->begin(), edgelist->end());
  }
  ...
  if (test_result) {
    ...
    char dummy_algo_data;
    auto tester = havoqgt::create_visitor_queue<tester_t, havoqgt::detail::visitor_priority_queue>(graph, dummy_algo_data);
