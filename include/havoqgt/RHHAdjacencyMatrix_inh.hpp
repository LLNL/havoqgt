...
class RHHAdjacencyMatrix {
  ...
  bool insert_uniquely(SegmentAllocator& allocator, KeyType key, ValueType val)
  {
  ...
  bool erase(SegmentAllocator& allocator, KeyType key, ValueType value)
  {
    const uint64_t mask = cal_mask();
  	ProbeDistanceType dist = 0;
    const int64_t pos_first = lookup_first_position(key, &dist, mask);
    ...
    if (is_adj_list(property(pos_first))) {
      ...
      if (rhh_adj_list.m_pos_head_->num_elems == kDirectInsertionThreshold) {
      	KeyType moved_value_list[kDirectInsertionThreshold];
...
#define USE_TOMBSTONE 1
  ...
  inline PropertyBlockType cal_property(HashType hash, const int64_t slot_index, const bool is_adj_list, const uint64_t mask)
  {
