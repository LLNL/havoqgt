...
#ifndef HAVOQGT_PARALLEL_EDGE_STREAM_READER_INCLUDED
#define HAVOQGT_PARALLEL_EDGE_STREAM_READER_INCLUDED
...
class parallel_edge_stream_reader {
 public:
  ...
  class input_iterator_type
   ...
   public:
    input_iterator_type(parallel_edge_stream_reader *ptr_reader, const bool end)
        : m_ptr_reader(ptr_reader), m_count(0), m_current(), m_make_undirected(false), m_end(end) {
      if (!m_end) {
        get_next();
    ...
    bool is_equal(const input_iterator_type &_x) const {
      return (m_end && _x.m_end) // Both are the end
             || ((!m_end && !_x.m_end) && m_count == (_x.m_count)); // Both are not at the end && count is the same
    }
   ...
   private:
    ...
    parallel_edge_stream_reader *m_ptr_reader;
    uint64_t                    m_count;
    edge_type                   m_current;
  ...
  parallel_edge_stream_reader(const std::vector<std::string> &filenames,
                              bool undirected)
  ...
  input_iterator_type begin() {
    ...
    open_files();
    return input_iterator_type(this, false);
  }

  /// Returns the end of the input iterator
  input_iterator_type end() {
    return input_iterator_type(this, true);
  }
...
#endif  // HAVOQGT_PARALLEL_EDGE_STREAM_READER_INCLUDED
