...
class no_delegate_vertex_locator {
 ...
 public:
  ...
  uint32_t owner() const {
    return hash() % havoqgt::mpi_comm_size();
  }
...
class dg_visitor {
 public:
  ...
  bool pre_visit(AlgData& algo_data) const {
  ...
  bool visit(const graphstore_type& graph, VisitorQueueHandle vis_queue, AlgData& algo_data) const {
    switch (vis_type) {
      case ADD: {
