...
#include <dynamic_graph_store/rhh/rhh_defs.hpp>
#include <dynamic_graph_store/rhh/rhh_utilities.hpp>
#include <dynamic_graph_store/rhh/rhh_container.hpp>
#include <dynamic_graph_store/rhh/blocked_rhh_container.hpp>
#include <dynamic_graph_store/rhh/rhh_allocator_holder.hpp>
...
class degawarerhh
{
 ...
 private:
  using in_core_allocater      = rhh::allocator_holder_sglt<rhh::ALLOC_IN_CORE, 0, 0>;
  using out_of_core_allocater  = rhh::allocator_holder_sglt<segment_manager_type, 0, 0>;
 ...
 public:
  ...
  bool erase_edge(const vertex_type& src, const vertex_type& trg)
  {
    ...
    auto itr_mhdeg_table = m_mhdeg_table->find(src);
    /// has source vertex ?
    if (itr_mhdeg_table.is_end()) return false;
    mhdeg_edge_chunk_type* edge_chunk = itr_mhdeg_table->second;
    /// has the target edge ?
    ...
    if (edge_chunk->size() < middle_high_degree_threshold) {
      const vertex_property_data_type& property_data = itr_mhdeg_table->first;
      bool is_first = true;
      ...
      m_mhdeg_table->erase(itr_mhdeg_table);
      rhh::shrink_to_fit(&m_mhdeg_table, 2.0);
    } else {
      rhh::shrink_to_fit(&edge_chunk, 2.0);
      itr_mhdeg_table->second = edge_chunk;
    }
  ...
  size_type erase_edge_dup(const vertex_type& src, const vertex_type& trg)
  {
    ...
    auto itr_mhdeg_table = m_mhdeg_table->find(src);
    /// has source vertex ?
    if (itr_mhdeg_table.is_end()) return 0;
    mhdeg_edge_chunk_type* edge_chunk = itr_mhdeg_table->second;
  ...
  inline vertex_property_data_type& vertex_property_data(const vertex_type& vertex)
  {
    ...
    auto itr_mhdeg_table = m_mhdeg_table->find(vertex);
    return itr_mhdeg_table->first;
  }
 ...
 private:
  ...
  {
    auto itr_mhdeg_table = m_mhdeg_table->find(vertex);
    if (!itr_mhdeg_table.is_end()) {
      const mhdeg_edge_chunk_type* const edge_chunk = itr_mhdeg_table->second;
  ...
  {
    const auto itr_mhdeg_table = m_mhdeg_table->find(src_vrt);
    if (!itr_mhdeg_table.is_end()) {
      mhdeg_edge_chunk_type* edge_chunk = itr_mhdeg_table->second;
...
template <typename _vertex_type,
          ...
          typename _edge_property_data_type,
          typename _segment_manager_type>
class degawarerhh <_vertex_type, _vertex_property_data_type, _edge_property_data_type, _segment_manager_type, 1>
{
 public:
  using vertex_type                 = _vertex_type;
  using vertex_property_data_type   = _vertex_property_data_type;
 ...
 public:
  ...
  inline std::pair<vertex_iterator, vertex_iterator> find_vertex(const vertex_type& vertex)
  {
    return std::make_pair(vertex_iterator(m_mhdeg_table->find(vertex)),
                          vertex_iterator(m_mhdeg_table->find_end()));
  }
  ...
  inline vertex_iterator vertices_begin()
  {
    return vertex_iterator(mhdeg_vertices_begin());
  }

  inline vertex_iterator vertices_end()
  {
    return vertex_iterator(mhdeg_vertices_end());
  }

  inline adjacent_edge_iterator adjacent_edge_begin(const vertex_type& src_vrtx)
  {
    return adjacent_edge_iterator(mhdeg_adjacent_edge_begin(src_vrtx));
  }

  inline adjacent_edge_iterator adjacent_edge_end()
  {
    return adjacent_edge_iterator(mhdeg_adjacent_edge_end());
  }
  ...
  inline size_type degree(const vertex_type& vertex)
  {
    return count_degree_mhdeg_table(vertex);;
  }
...
#include <dynamic_graph_store/degawarerhh/degawarerhh_vertex_iterator.hpp>
#include <dynamic_graph_store/degawarerhh/degawarerhh_adjacent_edge_iterator.hpp>
