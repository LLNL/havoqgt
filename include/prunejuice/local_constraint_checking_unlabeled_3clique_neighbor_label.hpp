...
class lppm_3clique_visitor {
public:
  typedef typename Graph::vertex_locator vertex_locator;
  typedef typename Graph::edge_iterator eitr_type;

  lppm_3clique_visitor() : 
    msg_type(0) {}

  lppm_3clique_visitor(vertex_locator _vertex, uint8_t _msg_type = 0) : 
    vertex(_vertex), 
    msg_type(_msg_type) {}

  lppm_3clique_visitor(vertex_locator _vertex, vertex_locator _parent, 
    size_t _parent_pattern_index, size_t _parent_degree, 
    VertexData _parent_vertex_data,
    uint8_t _msg_type) :
    vertex(_vertex),
    parent(_parent),
    parent_pattern_index(_parent_pattern_index),
    parent_degree(_parent_degree),
...
/*  template<typename ByteVector>
  ...
  lppm_3clique_visitor(vertex_locator _vertex, vertex_locator _parent, 
    BitSet _parent_template_vertices, size_t _parent_degree, 
    VertexData _parent_vertex_data,
    uint8_t _msg_type) : 
    vertex(_vertex), 
    parent(_parent),
  ...
  bool pre_visit(AlgData& alg_data) const {
    ...
    auto vertex_data = static_cast<VertexData>(1); //std::get<0>(alg_data)[vertex];
    ...
    // std::get<13>(alg_data) - active_edge_target_vertex_degree_map
    ...
    if (!match_found) { 
      ...
      if (msg_type == 0 && match_found) {
        ...
        for(eitr_type eitr = g.edges_begin(vertex);
          ...
          lppm_3clique_visitor new_visitor(neighbor, vertex, 
            vertex_template_vertices, g.degree(vertex), 
            std::get<0>(alg_data)[vertex], 1); // std::get<0>(alg_data)[vertex] - label of vertex
...
//        }
        ...
        lppm_3clique_visitor new_visitor(neighbor, vertex, 
          vertex_template_vertices, std::get<12>(alg_data)[vertex].size(), 
           std::get<0>(alg_data)[vertex], 1); // std::get<0>(alg_data)[vertex] - label of vertex
        vis_queue->queue_visitor(new_visitor);
...
//--      return true;
  ...
  friend inline bool operator>(const lppm_3clique_visitor& v1, const lppm_3clique_visitor& v2) {
    return false;
  }
...
//16MARCH2019        std::cerr << "Error: did not find the expected item in the map." << std::endl;
    ...
    // 3-Clique optimization
    if (parent_degree > 1) {
      std::get<13>(alg_data)[g->locator_to_label(parent)][0] = parent_degree;
      std::get<13>(alg_data)[g->locator_to_label(parent)][1] = parent_vertex_data; 
    } else {
      std::get<13>(alg_data).erase(g->locator_to_label(parent));
    }      

    // vertex data of the target vertex of an edge
//    std::get<13>(alg_data)[g->locator_to_label(parent)][1] = parent_vertex_data;
    // TODO: remove the target vertex from the map if it is not present in the active edges map
...
void verify_and_update_vertex_state_3clique(TGraph* g, AlgData& alg_data, 
    ...
    for (size_t vertex_pattern_index = 0; // TODO: change the name vertex_pattern_index to template_vertex
      ...
      if (v.second.template_vertices.test(vertex_pattern_index)) {
        ...
        /*for (auto e = pattern_graph.vertices[vertex_pattern_index];
          ...
          ////////////////////////////////////////////////////////////////////////
          // unlabeled
          /*if (vertex_pattern_index == 3 && vertex_active_edges_map[v_locator].size() < 1) {
            v.second.template_vertices.reset();    
          } else if (vertex_pattern_index == 0 && vertex_active_edges_map[v_locator].size() < 3) {
            v.second.template_vertices.reset(vertex_pattern_index);
          }  else if (vertex_active_edges_map[v_locator].size() < 2) {
            v.second.template_vertices.reset(vertex_pattern_index);
          }*/
          // unlabeled
          ////////////////////////////////////////////////////////////////////////  
        ...
        ////////////////////////////////////////////////////////////////////////
        // unlabeled
        if (vertex_active_edges_map[v_locator].size() < 2) { // 3-Clique optimization
          v.second.template_vertices.reset();    
        } 
        // unlabeled
        ////////////////////////////////////////////////////////////////////////
...
template <typename Vertex, typename VertexData, typename TGraph, 
  ...
  typename PatternGraph, typename VertexSizeMap>
void local_constraint_checking_3clique(TGraph* g, 
...
//  VertexStateMapGeneric vertex_state_map_generic;  
  ...
  typedef prunejuice::lppm_3clique_visitor<TGraph, Vertex, VertexData, BitSet> visitor_type;
  auto alg_data = std::forward_as_tuple(vertex_metadata, pattern, pattern_indices, vertex_rank,
    ...
    global_init_step, g, template_vertices, vertex_active_edges_map, active_edge_target_vertex_degree_map);
  ...
  do {
    ...
    prunejuice::verify_and_update_vertex_state_3clique<TGraph, decltype(alg_data), VertexStateMapGeneric,
