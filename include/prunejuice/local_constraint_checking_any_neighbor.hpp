...
///static uint64_t lppm_any_nbr_visitor_count = 0;
...
class lppm_any_nbr_visitor {
public:
  typedef typename Graph::vertex_locator vertex_locator;
  typedef typename Graph::edge_iterator eitr_type;

  lppm_any_nbr_visitor() : 
    msg_type(0) {}

  lppm_any_nbr_visitor(vertex_locator _vertex, uint8_t _msg_type = 0) : 
    vertex(_vertex), 
    msg_type(_msg_type) {}

  lppm_any_nbr_visitor(vertex_locator _vertex, vertex_locator _parent, 
...
/*  template<typename ByteVector>
  lppm_any_nbr_visitor(vertex_locator _vertex, ByteVector _parent_template_vertices, 
    uint8_t _msg_type) :
    vertex(_vertex),
    msg_type(_msg_type) {
    ...
    if (!match_found) { 
      ...
      if (msg_type == 0 && match_found) {
        ...
        for(eitr_type eitr = g.edges_begin(vertex);
          ...
          lppm_any_nbr_visitor new_visitor(neighbor, vertex, vertex_template_vertices, 1);
...
//--        vertex_locator neighbor = eitr.target();
      ...
      for (auto& item : std::get<12>(alg_data)[vertex]) { 
        ...
        //for (auto vertex_pattern_index : vertex_pattern_indices) {
          ...
          //lppm_any_nbr_visitor new_visitor(neighbor, vertex_pattern_index, 1);
          //vis_queue->queue_visitor(new_visitor);
        //} // for

//        for (size_t i = 0; i < vertex_template_vertices.size(); i++) { // TODO: temporary fix
//          if (vertex_template_vertices.test(i)) {
            //lppm_any_nbr_visitor new_visitor(neighbor, i, 1);
            //vis_queue->queue_visitor(new_visitor);
...
//        }
        ...
        //lppm_any_nbr_visitor new_visitor(neighbor, vertex_template_vertices_array, 1);
        lppm_any_nbr_visitor new_visitor(neighbor, vertex, vertex_template_vertices, 1); 
        vis_queue->queue_visitor(new_visitor);
...
//--      return true;
  ...
  friend inline bool operator>(const lppm_any_nbr_visitor& v1, const lppm_any_nbr_visitor& v2) {
    return false;
  }
...
void verify_and_update_vertex_state_any_neighbor(TGraph* g, AlgData& alg_data, 
  ...
  for (auto& v : vertex_state_map) {
    ...
    for (size_t vertex_pattern_index = 0; // TODO: change the name vertex_pattern_index to template_vertex
      ...
      if (v.second.template_vertices.test(vertex_pattern_index)) {
        ...
        if (tmp_bitset.any() && !tmp_bitset.none()) { // Important : approximate matching
...
void local_constraint_checking_any_neighbor(TGraph* g, 
...
//  VertexStateMapGeneric vertex_state_map_generic;  
  ...
  typedef prunejuice::approximate::lppm_any_nbr_visitor<TGraph, Vertex, VertexData, 
    BitSet> visitor_type;
  ...
  do {
    ...
    prunejuice::approximate::verify_and_update_vertex_state_any_neighbor<TGraph, decltype(alg_data), VertexStateMapGeneric,
    ...
    lppm_any_nbr_visitor_count = 0; // reset for next iteration 
