...
// visitor class
template<typename Graph, typename Vertex, typename VertexData, 
  typename VertexUintMap>
...
public:
  ...
  three_clique_visitor() : msg_type(0) {}

  three_clique_visitor(vertex_locator _vertex) :
    vertex(_vertex), msg_type(0) {}

  template <typename NeighborArrayStatic>
  three_clique_visitor(vertex_locator _vertex,
    vertex_locator _parent, NeighborArrayStatic& _parent_neighbors, 
    uint8_t _msg_type) : //, Vertex _third_vertex_label) : 
  ...
  /*template <typename NeighborArrayStatic>
  three_clique_visitor(vertex_locator _vertex,
    NeighborArrayStatic& _parent_neighbors, uint8_t _msg_type) :  
    vertex(_vertex), msg_type(_msg_type) {      
      std::copy(std::begin(_parent_neighbors), 
        std::end(_parent_neighbors), std::begin(parent_neighbors));    
    }*/
...
//#ifdef ENABLE_BLOCK 
    auto find_parent_tvm = std::get<3>(alg_data).find(g->locator_to_label(parent));
    if (find_parent_tvm == std::get<3>(alg_data).end()) {
      std::cerr << "Error: did not find the expected item in the map."
        << std::endl;
      return false; 
    }
    auto parent_vertex_data = find_parent_tvm->second[1];  

    //for (auto& q : parent_neighbors) {

      auto q = parent_neighbors[0];  
      ...
      if (find_q != std::get<1>(alg_data)[vertex].end()) {
        ...
        Vertex third_vertex_label = q; // vertex ID of a neighbor q
 
	auto find_q_tvm = std::get<3>(alg_data).find(q);
        if (find_q_tvm == std::get<3>(alg_data).end()) {
          std::cerr << "Error: did not find the expected item in the map."
           << std::endl;
          return false;
        }
        auto third_vertex_data = find_q_tvm->second[1];     
    ...
    if (find_edge == std::get<2>(alg_data)[vertex].end()) {
      ...
      find_edge->second.insert({parent_vertex_data, 1});
    } else {
      /*auto find_w = find_edge->second.find(g->locator_to_label(parent));
      if (find_w == find_edge->second.end()) {
        find_edge->second.insert({g->locator_to_label(parent), 1});
      }*/
      auto find_w = find_edge->second.find(parent_vertex_data);
      if (find_w == find_edge->second.end()) {
        find_edge->second.insert({parent_vertex_data, 1});
      }
    }  
...
//#endif
  ...
  bool visit(Graph& g, VisitorQueueHandle vis_queue, AlgData& alg_data) const {
    ...
    for (auto& p : std::get<1>(alg_data)[vertex]) {
      ...
      if (find_p == std::get<3>(alg_data).end()) {
        ...
        << std::endl;
        return false;
      } else if (vertex_degree > find_p->second[0]) {
        continue;
      } else if ((vertex_degree == find_p->second[0]) && (g.locator_to_label(vertex) > p.first)) {
        continue;
      } 
      ...
      for (auto& q : std::get<1>(alg_data)[vertex]) {
        ...
        if (find_q == std::get<3>(alg_data).end()) {
          ...
          << std::endl;
          return false;
        } else if (find_p->second[0] > find_q->second[0]) {
          item_count++;
          continue;
        } else if ((find_p->second[0] == find_q->second[0]) && (p.first > q.first)) {
          item_count++;
          continue;
        }
        ...
        if (msg_count >= (max_msg_items - 1)) {
          ...
          q_neighbors[msg_count] = q.first; 
          // send
          three_clique_visitor new_visitor(neighbor, vertex, q_neighbors, 1);
          vis_queue->queue_visitor(new_visitor);
          // reset
          q_neighbors.fill(std::numeric_limits<std::uint64_t>::max()); 
          msg_count = 0;  
    ...
    if (find_edge == std::get<2>(alg_data)[vertex].end()) {
      ...
      if (find_w == find_edge->second.end()) {
        ...
        //update the remote edge v -- w, v -- u -- w

        std::array<Vertex, max_msg_items> q_neighbors;
        q_neighbors.fill(std::numeric_limits<std::uint64_t>::max());
        q_neighbors[0] = third_vertex_label; // w
        //q_neighbors[1] = g.locator_to_label(vertex); // u
        three_clique_visitor new_visitor(parent, vertex, q_neighbors, 2); // parent - v
        vis_queue->queue_visitor(new_visitor);
  ...
  //VertexData third_vertex_data;
  std::array<Vertex, max_msg_items> parent_neighbors;
  uint8_t msg_type; // 0 - init, 1 - update remote edges, 2 - update local edges
...
void three_clique_edge_state_batch(TGraph* g, VertexMetadata& vertex_metadata,
  ...
  typedef three_clique_visitor<TGraph, Vertex, VertexData, 
    VertexUint8Map> visitor_type;
