...
///static uint64_t lppm_opt_nbr_visitor_count = 0;

///namespace havoqgt { ///namespace mpi {
namespace prunejuice { namespace approximate { 
...
class lppm_opt_nbr_visitor {
public:
  typedef typename Graph::vertex_locator vertex_locator;
  typedef typename Graph::edge_iterator eitr_type;

  lppm_opt_nbr_visitor() : 
    msg_type(0) {}

  lppm_opt_nbr_visitor(vertex_locator _vertex, uint8_t _msg_type = 0) : 
    vertex(_vertex), 
    msg_type(_msg_type) {}

  lppm_opt_nbr_visitor(vertex_locator _vertex, vertex_locator _parent, 
...
/*  template<typename ByteVector>
  lppm_opt_nbr_visitor(vertex_locator _vertex, ByteVector _parent_template_vertices, 
    uint8_t _msg_type) :
    vertex(_vertex),
    msg_type(_msg_type) {
...
//              } // for
        ...
        if (!match_found) {
          ...
          std::get<11>(alg_data)[vertex] = vertex_template_vertices.to_ulong(); // TODO: verify type compatibility 
    ...
    if (!match_found) { 
      ...
      if (msg_type == 0 && match_found) {
        ...
        for(eitr_type eitr = g.edges_begin(vertex);
          ...
          lppm_opt_nbr_visitor new_visitor(neighbor, vertex, vertex_template_vertices, 1);
...
//--        vertex_locator neighbor = eitr.target();
      ...
      for (auto& item : std::get<12>(alg_data)[vertex]) { 
        ...
        //for (auto vertex_pattern_index : vertex_pattern_indices) {
          ...
          //lppm_opt_nbr_visitor new_visitor(neighbor, vertex_pattern_index, 1);
          //vis_queue->queue_visitor(new_visitor);
        //} // for

//        for (size_t i = 0; i < vertex_template_vertices.size(); i++) { // TODO: temporary fix
//          if (vertex_template_vertices.test(i)) {
            //lppm_opt_nbr_visitor new_visitor(neighbor, i, 1);
            //vis_queue->queue_visitor(new_visitor);
...
//        }
        ...
        //lppm_opt_nbr_visitor new_visitor(neighbor, vertex_template_vertices_array, 1);
        lppm_opt_nbr_visitor new_visitor(neighbor, vertex, vertex_template_vertices, 1); 
        vis_queue->queue_visitor(new_visitor);
...
//--      return true;
  ...
  friend inline bool operator>(const lppm_opt_nbr_visitor& v1, const lppm_opt_nbr_visitor& v2) {
    return false;
  }
...
///    typedef vertex_state_generic<Vertex, VertexData, uint8_t, BitSet> VertexState;
    typedef vertex_state<Vertex, VertexData, BitSet> VertexState; // TODO: add header and namespace 
...
void verify_and_update_vertex_state_optional_neighbor(TGraph* g, AlgData& alg_data, 
  ...
  for (auto& v : vertex_state_map) {
    ...
    for (size_t vertex_pattern_index = 0; // TODO: change the name vertex_pattern_index to template_vertex
      ...
      if (v.second.template_vertices.test(vertex_pattern_index)) {
        ...
        // if did not receive from all the required neighbours, set the vertex_pattern_index bit in 
        // v.second.template_vertices to 0
        ...
        bool valid_neighbors_found = false; 

        //BitSet tmp_pattern_vertex_edges = pattern_graph.vertex_neighbor_bitset[vertex_pattern_index];  

        // TODO: this should be done once per MPI rank  
        /*for (auto e = pattern_graph.vertices[vertex_pattern_index];
          e < pattern_graph.vertices[vertex_pattern_index + 1]; e++) {
        ...
        assert(pattern_graph.vertex_neighbor_bitset[vertex_pattern_index]); // Important 
        ...
        //auto tmp_bitset = pattern_graph.vertex_mandatory_neighbor_bitset[vertex_pattern_index] & v.second.template_neighbors;

        //if (pattern_graph.vertex_mandatory_neighbor_bitset[vertex_pattern_index] == tmp_bitset && 
        //  !tmp_bitset.none()) { // Important : exact matching
        //if (tmp_pattern_vertex_edges == tmp_bitset && !tmp_bitset.none()) { // Important : exact matching
          // heard from all required neighbors 
        ...
        //if (tmp_bitset.any() && !tmp_bitset.none()) { // Important : approximate matching - edit distance
	  // heard from at least one valid neighbor
	   
        // Note : If a template vertex has at least one mandatory neighbor, 
        // we do not need to verify if it has valid optional neighbor(s). If the
        // vertex does not have any mandatory neighbor, it must hear from at 
        // least one valid neighbor     

	if (pattern_graph.vertex_mandatory_neighbor_bitset[vertex_pattern_index].none()) { 
          // template vertex does not have mandatory neighbors
          auto tmp_bitset = pattern_graph.vertex_neighbor_bitset[vertex_pattern_index] & 
            v.second.template_neighbors; // TODO: vertex_optional_neighbor_bitset?

          if (tmp_bitset.any() && !tmp_bitset.none()) {
            valid_neighbors_found = true;
          }   
      
        } else if (pattern_graph.vertex_mandatory_neighbor_bitset[vertex_pattern_index].any()) {
          // template vertex has mendatory neighbors 
          auto tmp_bitset = pattern_graph.vertex_mandatory_neighbor_bitset[vertex_pattern_index] & 
            v.second.template_neighbors;

          // must hear from all the mandatory neighbors  
          if (pattern_graph.vertex_mandatory_neighbor_bitset[vertex_pattern_index] == tmp_bitset && 
            !tmp_bitset.none()) {
            valid_neighbors_found = true;
          }           
        ...
        //} else {
          // did not hear from all required neighbors
          assert(vertex_pattern_index < 16); // Test 
...
void local_constraint_checking_optional_neighbors(TGraph* g, 
...
//  VertexStateMapGeneric vertex_state_map_generic;  
  ...
  typedef prunejuice::approximate::lppm_opt_nbr_visitor<TGraph, Vertex, VertexData, 
    BitSet> visitor_type;
  auto alg_data = std::forward_as_tuple(vertex_metadata, pattern, 
    pattern_indices, vertex_rank, vertex_active, vertex_iteration, 
    vertex_state_map_generic, pattern_graph, superstep_var, global_init_step, 
    g, template_vertices, vertex_active_edges_map); // TODO: clean up
  ...
  do {
    ...
    prunejuice::approximate::verify_and_update_vertex_state_optional_neighbor<TGraph, decltype(alg_data), VertexStateMapGeneric,
      PatternGraph, VertexActive, VertexIteration, BitSet, TemplateVertex>(g, alg_data, vertex_state_map_generic, pattern_graph, 
      vertex_active, vertex_iteration, superstep, global_init_step, global_not_finished, not_finished, template_vertices, 
    ...
    lppm_opt_nbr_visitor_count = 0; // reset for next iteration 
...
}} // end namespace prunejuice::approximate ///} //end namespace havoqgt::mpi
