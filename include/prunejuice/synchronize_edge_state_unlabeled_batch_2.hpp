...
static int min_mpi_rank = 0;
static int max_mpi_rank = 0;
...
public:
  ...
  //sync_edge_state_visitor(vertex_locator _vertex,
  //  vertex_locator _parent, Vertex _third_vertex_label) : 
  ...
  bool pre_visit(AlgData& alg_data) const {
    ...
    for (auto& q : parent_neighbors) {
      ...
      auto find_q = std::get<2>(alg_data)[vertex].find(q);
      if (find_q != std::get<2>(alg_data)[vertex].end()) {        
  ...
  bool visit(Graph& g, VisitorQueueHandle vis_queue, AlgData& alg_data) const {
    ...
    if ((mpi_rank < min_mpi_rank) && 
      (mpi_rank > max_mpi_rank)) {
      false; 
    }  
    ...
    /*for (auto& v : std::get<1>(alg_data)[vertex]) {
      ...
      for (auto&  w : v.second) {
        ...
        sync_edge_state_visitor new_visitor(neighbor, vertex, w.first);
    ...
    for (auto& v : std::get<1>(alg_data)[vertex]) {
      ...
      if (g.locator_to_label(vertex) > v.first) { // Test
        continue;
      }
      ...
      size_t msg_count = 0;
      size_t max_msg_count = 20; 

      std::array<Vertex, 20> q_neighbors;
      q_neighbors.fill(std::numeric_limits<std::uint64_t>::max());

      for (auto&  w : v.second) { 

      if (msg_count >= (max_msg_count - 1)) {
        q_neighbors[msg_count] = w.first;
        sync_edge_state_visitor new_visitor(neighbor, vertex, q_neighbors);
        vis_queue->queue_visitor(new_visitor);
        q_neighbors.fill(std::numeric_limits<std::uint64_t>::max());
        msg_count = 0;        
      } else if (msg_count < (max_msg_count - 1)) {
        q_neighbors[msg_count] = w.first;
        msg_count++;
      }
      ...
      if ((msg_count > 0) && (msg_count <= max_msg_count - 1)) {
        sync_edge_state_visitor new_visitor(neighbor, vertex, q_neighbors);
        vis_queue->queue_visitor(new_visitor);
      }
  ...
  std::array<Vertex, 20> parent_neighbors;
...
void synchronize_edge_state_batch(TGraph* g, VertexMetadata& vertex_metadata,
  ...
  for (auto batch_mpi_rank = 0;  batch_mpi_rank < max_batch_size;
    ...
    min_mpi_rank = batch_mpi_rank;
    max_mpi_rank = batch_max_mpi_rank;
