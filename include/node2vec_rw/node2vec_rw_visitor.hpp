...
class rw_visitor_data {
 private:
  ...
  using negithbor_selector_type = neighbor_selector_with_rejection_sampling<graph_allocator_type,
                                                                            edge_weight_data_type>;
  using walk_log_type = std::tuple<uint64_t, uint16_t, vertex_type>;
 ...
 public:
  ...
  rw_visitor_data(graph_type &graph,
      ...
      : m_graph(graph),
        ...
        m_negithbor_selector(graph, edge_weight_data, level_diff_table, p, q, small_edge_weight_variance),
        m_walk_log_store(),
        m_mpi_comm(mpi_comm) {
    clear();
  }
  ...
  uint16_t walk_length() const {
    return m_walk_length;
  }

  int gen_walker_id() {
    const uint64_t max = (1ULL << 40ULL) - 1;
  ...
  void store(const uint64_t walker_id, const uint8_t walk_step, const vertex_type &vertex) {
    m_walk_log_store.push_back(std::make_tuple(walker_id, 0, vertex));
  }

  vertex_type select_neighbor(const vertex_type &prev_vertex,
                              const vertex_type &current_vertex,
                              const bool first_step) {
    return m_negithbor_selector.select(prev_vertex, current_vertex, first_step, &m_rnd_generator);
  }
 ...
 private:
  walk_log_store_type shuffle_walk_log() const {
    ...
    auto walk_log_partitioner = [mpi_size](const walk_log_type &data) -> int {
      return node2vec_rw::hash<typename std::tuple_element<walk_log::walk_id, walk_log_type>::type>{}
          (std::get<walk_log::walk_id>(data)) % mpi_size;
    };
  ...
  const uint16_t m_walk_length;
  uint64_t m_walker_id;
  rand_1024 m_rnd_generator;
  negithbor_selector_type m_negithbor_selector;
...
class node2vec_rw_visitor {
 ...
 public:
  ...
  node2vec_rw_visitor(const vertex_type _vertex, const vertex_type _previous_vertex,
                      const uint64_t _wk_id, const uint16_t _walk_length)
      : vertex(_vertex),
  ...
  bool init_visit(graph_type &g, VisitorQueueHandle vis_queue, algorithm_data_type &algorithm_data) const {
    ...
    const uint64_t walker_id = algorithm_data.gen_walker_id();
