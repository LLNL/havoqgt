#ifndef HAVOQGT_VERTEX_DATA_DB_SAME_LABEL_HPP_INCLUDED
#define HAVOQGT_VERTEX_DATA_DB_SAME_LABEL_HPP_INCLUDED
...
class vertex_data_same_label_queue {

public:
  vertex_data_same_label_queue() {}
...
class vertex_data_same_label_visitor {
public:
  typedef typename Graph::vertex_locator vertex_locator;
  typedef typename Graph::edge_iterator eitr_type;

  vertex_data_same_label_visitor() : 
    vertex_data(0),
    do_update_vertex_data(false) {}

  vertex_data_same_label_visitor(vertex_locator _vertex) : 
    vertex(_vertex), 
    vertex_data(0), 
    do_update_vertex_data(false) {}

  vertex_data_same_label_visitor(vertex_locator _vertex, VertexData _vertex_data) :
    vertex(_vertex), 
  ...
  bool visit(Graph& g, VisitorQueueHandle vis_queue, AlgData& alg_data) const {
    ...
    // random label
    std::get<0>(alg_data)[vertex] = std::get<2>(alg_data)(std::get<1>(alg_data));

    //std::cout << "Visiting " << g.locator_to_label(vertex) << " " << std::get<0>(alg_data)[vertex] << std::endl; // test
    return true;
  } 

  friend inline bool operator>(const vertex_data_same_label_visitor& v1, const vertex_data_same_label_visitor& v2) {
    return false;
  }
...
template <typename TGraph, typename VertexMetaData, typename Vertex, 
  typename VertexData>
void vertex_data_db_same_label(TGraph* g, VertexMetaData& vertex_metadata) {
  ///int mpi_rank = havoqgt_env()->world_comm().rank();
  int mpi_rank = comm_world().rank();

  if (mpi_rank == 0) {
    std::cout << "Building distributed vertex data (random label) db ... " << std::endl;
  }

  std::random_device rd;  //Will be used to obtain a seed for the random number engine
  std::mt19937 gen(rd()); //Standard mersenne_twister_engine seeded with rd()
  std::uniform_int_distribution<VertexData> distrib(1, 50);

  typedef vertex_data_same_label_visitor<TGraph, VertexData> visitor_type;
  auto alg_data = std::forward_as_tuple(vertex_metadata, gen, distrib);
