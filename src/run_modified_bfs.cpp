...
#include <havoqgt/breadth_first_search_modified.hpp>
...
int main(int argc, char** argv) {
  ...
  // BFS Experiments
  {
    ...
    bfs_level_data.reset(std::numeric_limits<uint64_t>::max());
    graph_type::vertex_data<graph_type::vertex_locator, std::allocator<graph_type::vertex_locator> >  bfs_parent_data(*graph);
    graph_type::vertex_data<graph_type::vertex_locator, std::allocator<graph_type::vertex_locator> > bfs_source_data(*graph);
    MPI_Barrier(MPI_COMM_WORLD);
    if (mpi_rank == 0) {
    ...
    uint64_t isource = source_vertex;
      ...
      hmpi::breadth_first_search(graph, bfs_level_data, bfs_parent_data, bfs_source_data,
          source);
	MPI_Barrier(MPI_COMM_WORLD);
      ...
      for (uint64_t level = 0; level < std::numeric_limits<uint16_t>::max(); ++level) {
        ...
        for (vitr = graph->vertices_begin();
          ...
          if (bfs_level_data[*vitr] == level) {
            ++local_count;
	    if(level == 4) {
	      std::cout << graph->locator_to_label(bfs_source_data[*vitr]) << " ";
	    }
          }
        }
	//std::cout << std::endl;
