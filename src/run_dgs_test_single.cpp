...
void parse_cmd_line(int argc, char **argv, std::string &segmentfile_name) {
  std::cout << "CMD line:";
  ...
  while ((c = getopt(argc, argv, "o:e:h")) != -1) {
    switch (c) {
      case 'h':prn_help = true;
        break;
      case 'o':segmentfile_name = optarg;
        break;
...
namespace std {
template <>
struct hash<std::pair<vertex_id_type, vertex_id_type>> {
...
int main(int argc, char **argv) {
  ...
  havoqgt::init(&argc, &argv);
  {
    ...
    parse_cmd_line(argc, argv, segmentfile_name);
    ...
    if (middle_high_degree_threshold > 1)
      run_time("can handle basic operations on a low degree graph?",
               test1(graphstore, 4, middle_high_degree_threshold - 1));
    run_time("can handle basic operations on a middle-high degree graph?",
             test1(graphstore, 4, middle_high_degree_threshold * 2)); // There is a possibility of long probedistances

    if (middle_high_degree_threshold > 1)
      run_time("can handle duplicated edges on a low degree graph?",
               test2(graphstore, 4, 1, middle_high_degree_threshold - 1));
    run_time("can handle duplicated edges on a middle-high degree graph?",
             test2(graphstore, 4, middle_high_degree_threshold * 2, 64));
