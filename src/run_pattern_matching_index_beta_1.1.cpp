...
//#include <prunejuice/local_constraint_checking.hpp> // replaced
...
// indexing
#include <prunejuice/local_constraint_checking_unlabeled_3clique_neighbor_label.hpp> 
//#include <prunejuice/three_clique_edge_state_batch.hpp> // labeled, incomplete
#include <prunejuice/three_clique_edge_state_batch_2.hpp>
#include <prunejuice/synchronize_edge_state_batch.hpp>

#include <prunejuice/ktruss.hpp>
#include <prunejuice/kclique.hpp>

#include <prunejuice/local_constraint_checking_neighbor_label.hpp>
#include <prunejuice/local_edge_constraint_checking.hpp>
...
*/
  ...
  std::string result_dir = "";//"/p/lustre3/havoqgtu/reza2_tmp/tmp_Twitter/tmp/"; 
  //"/p/lustre2/reza2/small_test_graphs/"; //"/todo/"; // TODO:  
...
///  typedef prunejuice::vertex_state_generic<Vertex, VertexData, uint8_t, BitSet> VertexState;
  ...
  typedef std::unordered_map<Vertex, std::array<uint64_t, 2> > VertexSizeMap;

  typedef VertexUint8MapCollection VertexActiveEdgeCollection; 
  ...
  VertexSizeMap active_edge_target_vertex_degree_map; // temporary hack 
...
//  VertexIteration vertex_iteration(*graph);
  ...
  bool is_no_vertex_metadata = true; //true;

  // write vertex data to file
  bool do_output_metadata = false; // false; // TODO: ?
  ...
  if (do_output_metadata) {
    ...
    for (vitr_type vitr = graph->vertices_begin(); vitr != graph->vertices_end();
      ...
      vertex_data_file << graph->locator_to_label(vertex) << " " << vertex_metadata[vertex] << "\n"; 
    } 	
  ...
  bool do_subgraph_indexing = true; // TODO: Boolean

  bool do_edge_constarint_checking = true;
  ...
  for (size_t pk_tmp = 0; pk_tmp <= edit_distance_prototype_vector.size(); 
    ...
    if (pk_tmp > 0) {
      ...
      do_subgraph_indexing = false;       
      do_edge_constarint_checking = true;  

      // indexing
      if (pk < 1) {
        if (mpi_rank == 0) { 
          std::cout << "Pattern Matching | Skipping Edit Distance "
            << pk << " Prototypes" << std::endl;
        } 
        continue;
      } 
      ...
      if (!do_subgraph_indexing) {
        if (mpi_rank == 0) {
          std::cout << "Pattern Matching | Skipping Subgraph Indexing"
            << std::endl; 
        }
        do_nonlocal_constraint_checking = false;
        do_edge_constarint_checking = false;
        continue; 
      }    
    ...
    if (mpi_rank == 0) { // TODO: 
      if (do_generate_max_candidate_set) {
        ...
        std::cout << "Pattern Matching | Subgraph Indexing ... " << std::endl;
      } else {  
        std::cout << "Pattern Matching | Searching Edit Distance " 
          << current_edit_distance << " Prototypes ... " << std::endl;  
        
        // indexing  
        if (current_edit_distance < 1) {
          if (mpi_rank == 0) { 
            std::cout << "Pattern Matching | Skipping Edit Distance "
              << current_edit_distance << " Prototypes" << std::endl; 
          }  
          //continue; // freezes, why ? 
        } 
...
/*  if (ps == 0) { 
  ...
  PatternNonlocalConstraint ptrn_util_two(pattern_graph,
   pattern_input_dir + "/pattern_nonlocal_constraint", 
   pattern_input_dir + "/pattern_edge_constraint");
  ...
  //vertex_active_edge_state_map.clear(); // TODO: do not reset 
...
//    global_itr_count, superstep_result_file, active_vertices_count_result_file, edge_active/**edge_data_ptr*/, edge_metadata);
 ...
 if (do_generate_max_candidate_set) {
   ...
   if(mpi_rank == 0) {
     ...
     //for (auto& e : active_edge_target_vertex_degree_map) {
     //  std::cout<< e.first << " " << e.second[0] << " " << e.second[1] << std::endl;
     //}
 ...
 prunejuice::label_propagation_pattern_matching_bsp<Vertex, VertexData, 
   graph_type, VertexMetadata, VertexStateMap, VertexActive, 
   VertexUint8MapCollection, VertexSizeMap, 
   TemplateVertexBitSet, TemplateVertex, PatternGraph>
   (graph, vertex_metadata, vertex_state_map, vertex_active, 
   vertex_active_edges_map, active_edge_target_vertex_degree_map,
   template_vertices, pattern_graph, global_init_step, 
   global_not_finished, global_itr_count, superstep_result_file, 
   active_vertices_count_result_file, active_edges_count_result_file,
   message_count_result_file);
...
//    global_active_vertex = false;
  ...
  if(mpi_rank == 0) {
     std::cout << " - | Edge Target Vertex Count : " << active_edge_target_vertex_degree_map.size() << std::endl;
     //for (auto& v : active_edge_target_vertex_degree_map) {
...
*/
  ...
  // Test
  //if(mpi_rank == 0) {
  //  std::cout << " - | Edge State Map Size : " << vertex_active_edge_state_map.size() << std::endl;
  //}
  ...
  // k-truss - supports only the unlabeled case 
  ...
  // k-clique - supports only the unlabeled case
  ...
  // edge constraint checking

  //do_edge_constarint_checking = false; // Test

  if (do_edge_constarint_checking && global_not_finished) { //&& 
    //(ptrn_util_two.edge_constraints.size() > 0)) { 
 
    auto pattern_edge_constraints = ptrn_util_two.edge_constraints;

    // Test  
    if (mpi_rank == 0) { 
      std::cout << "Pattern Matching | Edge Constraints : " << std::endl;
      if (ptrn_util_two.edge_constraints.size() < 1) {
        std::cout << "None" << std::endl;
      } 
      for (size_t i = 0; i < ptrn_util_two.edge_constraints.size(); i++) {
        auto u = std::get<0>(ptrn_util_two.edge_constraints[i]);
        auto v = std::get<1>(ptrn_util_two.edge_constraints[i]); 
        auto w_vertices = std::get<2>(ptrn_util_two.edge_constraints[i]);
        std::cout << u << " " << v << " : ";
        for (auto& w : w_vertices) {
          std::cout << w << "; " << pattern_graph.vertex_data[w];
        }
        std::cout << std::endl;   
      } 
    }
    ...
    prunejuice::indexing::local_edge_constraint_checking 
      <graph_type, vitr_type, Vertex, VertexData, VertexMetadata, VertexActive, 
      VertexStateMap, VertexActiveEdgeCollection, EdgeStateCollection, 
      PatternGraph, PatternNonlocalConstraint, VertexUint8Map, VertexSizeMap, 
      TemplateVertex>
      (graph, vertex_metadata, vertex_active, vertex_state_map, 
      vertex_active_edges_map, vertex_active_edge_state_map, 
      pattern_graph, ptrn_util_two, active_edge_target_vertex_degree_map, 
      template_vertices); 

    MPI_Barrier(MPI_COMM_WORLD); // TODO: might not need this here
    time_end = MPI_Wtime();
    if(mpi_rank == 0) {
      std::cout << "Pattern Matching Time | Edge Constraint Checking : "
        << time_end - time_start << std::endl;
    }  
  ...
  label_propagation_time_end = MPI_Wtime();
  if(mpi_rank == 0) {
    std::cout << "Pattern Matching Time | Local Constraint Checking (Post Edge Constraint Checkng) : "
      << label_propagation_time_end - label_propagation_time_start << std::endl;
  }
