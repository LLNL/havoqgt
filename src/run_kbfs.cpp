...
template <typename vertex_locator_t>
class hash_vertex_locator
{
 public:
  std::size_t operator()(const vertex_locator_t& locator) const
  {
    using std::hash;
    return (hash<decltype(locator.owner())>()(locator.owner()) ^
            hash<decltype(locator.local_id())>()(locator.local_id()));
  }
};
...
int main(int argc, char** argv) {
  ...
  havoqgt::havoqgt_init(&argc, &argv);
  {
    ...
    // BFS Experiments
    {
      ...
      typedef fixed_size_unordered_map<graph_type::vertex_locator,
                                       uint16_t,
                                       hash_vertex_locator<graph_type::vertex_locator>> bfs_level_data_elemt_t;
      graph_type::vertex_data<bfs_level_data_elemt_t, std::allocator<bfs_level_data_elemt_t> > bfs_level_data(*graph);

      typedef fixed_size_unordered_map<graph_type::vertex_locator,
                                       graph_type::vertex_locator,
                                       hash_vertex_locator<graph_type::vertex_locator>> bfs_parent_data_elemt_t;
      graph_type::vertex_data<bfs_parent_data_elemt_t, std::allocator<bfs_parent_data_elemt_t> > bfs_parent_data(*graph);
      MPI_Barrier(MPI_COMM_WORLD);
      if (mpi_rank == 0) {
      ...
      {
        bfs_level_data_elemt_t init(source_list.size());
        for (const auto& s : source_list) {
          init[s] = std::numeric_limits<uint16_t>::max();
        }
        bfs_level_data.reset(init);
      }
      {
        bfs_parent_data_elemt_t init(source_list.size());
        bfs_parent_data.reset(init);
      }
      ...
      for (const auto& source : source_list) {
        if (mpi_rank == 0) {
          std::cout << "\nSource = " << graph->locator_to_label(source) << std::endl;
        }
        ...
        for (uint64_t level = 0; level < std::numeric_limits<uint16_t>::max(); ++level) {
          ...
          for (vitr = graph->vertices_begin();
               ...
               ++vitr) {
            if (bfs_level_data[*vitr][source] == level) {
              ++local_count;
            }
          }
          ...
          for (citr = graph->controller_begin();
               ...
               ++citr) {
            if (bfs_level_data[*citr][source] == level) {
              ++local_count;
            }
          }
