...
// g++ -std=c++11 -O3 -fopenmp ../src/extract_kth_core.cpp -o extract_kth_core
...
/// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
/// This program assume that input graph is a undirected and does not have self-loop and duplicated edges
/// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
size_t compute_k_core(const graph_bank<max_vid, 256> &graph,
                      ...
                      std::vector<uint64_t> &degree)
{
  ...
  while (true) {
    ...
    for (vid_t vid = 0; vid < actual_max_vid + 1; ++vid) {
      if (degree[vid] == 0) continue;

      if (degree[vid] < k) {
        degree[vid] = 0;  // kill the vertex
        ++total_num_killed;
        for (vid_t trg : graph[vid]) { // remove edges
          if (degree[trg] > 0) --degree[trg];
        }
        someone_killed = true;
      }
...
#pragma omp parallel for reduction(+:count_edges), reduction(max:actual_max_vid)
  ...
  const size_t num_killed = compute_k_core(graph, actual_max_vid, k, degree);
  ...
  for (size_t i = 0; i < edge_list_file.size(); ++i) {
    ...
    for (vid_t src = start; src <= end; ++src) {
      for (const auto &dst : graph[src]) {
        if (degree[src] == 0 || degree[dst] == 0) continue;
        if (src == dst) continue;
        if (src != dst && hash32(src) == hash32(dst)) {
